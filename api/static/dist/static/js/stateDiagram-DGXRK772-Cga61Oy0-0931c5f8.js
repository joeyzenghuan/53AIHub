import { X as O, J as M, q as P } from "./chunk-AEK57VVT-BGHUhwEk-239c44b3.js";
import { _ as f, c as t, l as E, d as R, j as H, P as U, Q as I, u as C } from "./mermaid-CI9Xz-tl-06ae91ae.js";
import { F as v } from "./viewer-DpLhQV_x-5cf483f8.js";
import { L as F } from "./graph-S7kq1CHN-26cf87d4.js";
import { o as J } from "./layout-4labnYEG-b2dd9f73.js";
import "./chunk-RZ5BOZE2-D7vSOP4k-fa3f167d.js";
...
var T = {}, W = f((e, n) => {
  T[e] = n;
}, "set"), $ = f((e) => T[e], "get"), A = f(() => Object.keys(T), "keys"), j = f(() => A().length, "size"), X = { get: $, set: W, keys: A, size: j }, q = f((e) => e.append("circle").attr("class", "start-state").attr("r", t().state.sizeUnit).attr("cx", t().state.padding + t().state.sizeUnit).attr("cy", t().state.padding + t().state.sizeUnit), "drawStartState"), Y = f((e) => e.append("line").style("stroke", "grey").style("stroke-dasharray", "3").attr("x1", t().state.textHeight).attr("class", "divider").attr("x2", t().state.textHeight * 2).attr("y1", 0).attr("y2", 0), "drawDivider"), _ = f((e, n) => {
...
}, "drawNote"), L = f(function(e, n) {
  const o = n.id, r = { id: o, label: n.id, width: 0, height: 0 }, d = e.append("g").attr("id", o).attr("class", "stateGroup");
  n.type === "start" && q(d), n.type === "end" && K(d), (n.type === "fork" || n.type === "join") && V(d, n), n.type === "note" && et(n.note.text, d), n.type === "divider" && Y(d), n.type === "default" && n.descriptions.length === 0 && _(d, n), n.type === "default" && n.descriptions.length > 0 && Q(d, n);
  const h = d.node().getBBox();
  return r.width = h.width + 2 * t().state.padding, r.height = h.height + 2 * t().state.padding, X.set(o, r), r;
  ...
  const d = n.points, h = U().x(function(s) {
    ...
    return s.y;
  }).curve(I), l = e.append("path").attr("d", h(d)).attr("id", "edge" + D).attr("class", "transition");
  let a = "";
  if (t().state.arrowMarkerAbsolute && (a = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, a = a.replace(/\(/g, "\\("), a = a.replace(/\)/g, "\\)")), l.attr("marker-end", "url(" + a + "#" + r(M.relationType.DEPENDENCY) + "End)"), o.title !== void 0) {
    const s = e.append("g").attr("class", "stateLabel"), { x, y: c } = C.calcLabelPosition(n.points), g = H.getRows(o.title);
    let u = 0;
    const B = [];
    let m = 0, N = 0;
  ...
  const a = new F({ compound: true, multigraph: true });
  let s, x = true;
  ...
  for (const i of u) {
    ...
    if (y.doc) {
      ...
      p = L(n, y, a);
    if (y.note) {
      const w = { descriptions: [], id: y.id + "-note", note: y.note, type: "note" }, k = L(n, w, a);
...
}, "renderDoc"), st = { setConf: nt, draw: dt }, wt = { parser: O, get db() {
  return new M(1);
}, renderer: st, styles: P, init: f((e) => {
