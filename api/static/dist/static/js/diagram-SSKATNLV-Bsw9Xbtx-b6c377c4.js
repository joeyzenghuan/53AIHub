import { m as S } from "./chunk-4BMEZGHF-BCJ74EV_-576729df.js";
import { E as I, _ as l, b as E, a as F, o as G, p as R, g as z, s as D, l as P, C as y, D as w, x as j, G as W, I as B } from "./mermaid-CI9Xz-tl-06ae91ae.js";
import { u as H } from "./mermaid-parser.core-DZwaOB10-cee09aad.js";
...
var x = { showLegend: true, ticks: 5, max: null, min: 0, graticule: "circle" }, b = { axes: [], curves: [], options: x }, m = structuredClone(b), V = I.radar, _ = l(() => y({ ...V, ...w().radar }), "getConfig"), C = l(() => m.axes, "getAxes"), q = l(() => m.curves, "getCurves"), U = l(() => m.options, "getOptions"), Y = l((a) => {
  m.axes = a.map((t) => ({ name: t.name, label: t.label ?? t.name }));
}, "setAxes"), Z = l((a) => {
  m.curves = a.map((t) => ({ name: t.name, label: t.label ?? t.name, entries: J(t.entries) }));
}, "setCurves"), J = l((a) => {
  if (a[0].axis == null)
...
}, "clear"), f = { getAxes: C, getCurves: q, getOptions: U, setAxes: Y, setCurves: Z, setOptions: K, getConfig: _, clear: N, setAccTitle: E, getAccTitle: F, setDiagramTitle: G, getDiagramTitle: R, getAccDescription: z, setAccDescription: D }, Q = l((a) => {
  S(a, f);
  const { axes: t, curves: e, options: r } = a;
  f.setAxes(t), f.setCurves(e), f.setOptions(r);
}, "populate"), X = { parse: l(async (a) => {
  const t = await H("radar", a);
  P.debug(t), Q(t);
}, "parse") }, tt = l((a, t, e, r) => {
  const i = r.db, n = i.getAxes(), o = i.getCurves(), s = i.getOptions(), c = i.getConfig(), d = i.getDiagramTitle(), g = W(t), u = et(g, c), p = s.max ?? Math.max(...o.map((v) => Math.max(...v.entries))), h = s.min, $ = Math.min(c.width, c.height) / 2;
...
function M(a, t, e, r, i, n, o) {
  ...
  e.forEach((d, g) => {
    ...
    const u = d.entries.map((p, h) => {
      const $ = 2 * Math.PI * h / s - Math.PI / 2, v = L(p, r, i, c), O = v * Math.cos($), k = v * Math.sin($);
      return { x: O, y: k };
    });
...
var it = { draw: tt }, st = l((a, t) => {
  ...
  const t = B(), e = w(), r = y(t, e.themeVariables), i = y(r.radar, a);
  ...
  return `
	...
	${st(t, e)}
	`;
}, "styles"), $t = { parser: X, db: f, renderer: it, styles: ot };
export {
  $t as diagram
};
