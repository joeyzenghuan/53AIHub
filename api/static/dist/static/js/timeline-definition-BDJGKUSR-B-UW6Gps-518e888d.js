import { _ as r, A as mt, X as xt, x as bt, c as kt, l as S, T as _t, U as vt, V as wt, W as St } from "./mermaid-j5R1o_wi-141fd499.js";
import { L as U } from "./helper-7WMIPux3-736d9956.js";
import { v as et } from "./arc-6Ea6x5Wp-e262708f.js";
...
var G = function() {
  var n = r(function(x, i, c, d) {
    ...
    var i = this, c = [0], d = [], u = [null], l = [], b = this.table, k = "", $ = 0, F = 0, O = 2, A = 1, N = l.slice.call(arguments, 1), m = Object.create(this.lexer), v = { yy: {} };
    ...
    function B() {
      var E;
      return E = d.pop() || m.lex() || A, typeof E != "number" && (E instanceof Array && (d = E, E = d.pop()), E = i.symbols_[E] || E), E;
    }
    r(B, "lex");
    for (var w, P, T, Y, C = {}, W, M, tt, V; ; ) {
      if (P = c[c.length - 1], this.defaultActions[P] ? T = this.defaultActions[P] : ((w === null || typeof w > "u") && (w = B()), T = b[P] && b[P][w]), typeof T > "u" || !T.length || !T[0]) {
        var Z = "";
        V = [];
        for (W in b[P])
...
` + m.showPosition() + `
Expecting ` + V.join(", ") + ", got '" + (this.terminals_[w] || w) + "'" : Z = "Parse error on line " + ($ + 1) + ": Unexpected " + (w == A ? "end of input" : "'" + (this.terminals_[w] || w) + "'"), this.parseError(Z, { text: m.match, token: this.terminals_[w] || w, line: m.yylineno, loc: L, expected: V });
      }
      if (T[0] instanceof Array && T.length > 1)
        throw new Error("Parse Error: multiple actions possible at state: " + P + ", token: " + w);
      switch (T[0]) {
        case 1:
          c.push(w), u.push(m.yytext), l.push(m.yylloc), c.push(T[1]), w = null, F = m.yyleng, k = m.yytext, $ = m.yylineno, L = m.yylloc;
          break;
        case 2:
          if (M = this.productions_[T[1]][1], C.$ = u[u.length - M], C._$ = { first_line: l[l.length - (M || 1)].first_line, last_line: l[l.length - 1].last_line, first_column: l[l.length - (M || 1)].first_column, last_column: l[l.length - 1].last_column }, R && (C._$.range = [l[l.length - (M || 1)].range[0], l[l.length - 1].range[1]]), Y = this.performAction.apply(C, [k, F, $, v.yy, T[1], u, l].concat(N)), typeof Y < "u")
            return Y;
          M && (c = c.slice(0, -1 * M * 2), u = u.slice(0, -1 * M), l = l.slice(0, -1 * M)), c.push(this.productions_[T[1]][0]), u.push(C.$), l.push(C._$), tt = b[c[c.length - 2]][c[c.length - 1]], c.push(tt);
          break;
        case 3:
          return true;
      }
...
var D = "", at = 0, J = [], X = [], z = [], ot = r(() => xt, "getCommonDb"), ct = r(function() {
  J.length = 0, X.length = 0, D = "", z.length = 0, bt();
  ...
  const a = { id: at++, section: D, type: D, task: n, score: t || 0, events: e ? [e] : [] };
  z.push(a);
}, "addTask"), pt = r(function(n) {
  z.find((t) => t.id === at - 1).events.push(n);
}, "addEvent"), yt = r(function(n) {
  const t = { section: D, type: D, description: n, task: n, classes: [] };
  X.push(t);
  ...
  for (const [e, a] of z.entries())
    n(e), t = t && a.processed;
  return t;
}, "compileTasks"), Et = { clear: ct, getCommonDb: ot, addSection: lt, getSections: ht, getTasks: dt, addTask: ut, addTaskOrg: yt, addEvent: pt }, Tt = 12, q = r(function(n, t) {
...
}, "drawLabel"), At = r(function(n, t, e) {
  const a = n.append("g"), s = K();
  s.x = t.x, s.y = t.y, s.fill = t.fill, s.width = e.width, s.height = e.height, s.class = "journey-section section-type-" + t.num, s.rx = 3, s.ry = 3, q(a, s), ft(e)(t.text, a, s.x, s.y, s.width, s.height, { class: "journey-section section-type-" + t.num }, e, t.colour);
  ...
  const o = K();
  o.x = t.x, o.y = t.y, o.fill = t.fill, o.width = e.width, o.height = e.height, o.class = "task task-type-" + t.num, o.rx = 3, o.ry = 3, q(s, o), ft(e)(t.task, s, o.x, o.y, o.width, o.height, { class: "task" }, e, t.colour);
}, "drawTask"), Ht = r(function(n, t) {
  q(n, { x: t.startx, y: t.starty, width: t.stopx - t.startx, height: t.stopy - t.starty, fill: t.fill, class: "rect" }).lower();
}, "drawBackgroundRect"), Ot = r(function() {
...
var Ct = r(function(n, t, e, a) {
...
}, "defaultBkg"), H = { drawRect: q, drawCircle: Lt, drawSection: At, drawText: gt, drawLabel: Mt, drawTask: Pt, drawBackgroundRect: Ht, getTextObj: Ot, getNoteRect: K, initGraphics: Rt, drawNode: Ct, getVirtualNodeHeight: Dt }, Ft = r(function(n, t, e, a) {
  var s, h;
  ...
  i.forEach(function(m) {
    const v = { number: $, descr: m, section: $, width: 150, padding: 20, maxHeight: c }, I = H.getVirtualNodeHeight(y, v, o);
    S.debug("sectionHeight before draw", I), c = Math.max(c, I + 20);
  });
  let O = 0, A = 0;
  S.debug("tasks.length", _.length);
  for (const [m, v] of _.entries()) {
    const I = { number: m, descr: v, section: v.section, width: 150, padding: 20, maxHeight: d }, L = H.getVirtualNodeHeight(y, I, o);
    S.debug("taskHeight before draw", L), d = Math.max(d, L + 20), O = Math.max(O, v.events.length);
    let R = 0;
    for (const j of v.events) {
      const B = { descr: j, section: v.section, number: v.section, width: 150, padding: 20, maxHeight: 50 };
      R += H.getVirtualNodeHeight(y, B, o);
    }
    A = Math.max(A, R);
  }
  S.debug("maxSectionHeight before draw", c), S.debug("maxTaskHeight before draw", d), i && i.length > 0 ? i.forEach((m) => {
    const v = _.filter((j) => j.section === m), I = { number: $, descr: m, section: $, width: 200 * Math.max(v.length, 1) - 50, padding: 20, maxHeight: c };
    S.debug("sectionNode", I);
    const L = y.append("g"), R = H.drawNode(L, I, $, o);
    S.debug("sectionNode output", R), L.attr("transform", `translate(${b}, ${l})`), k += c + 50, v.length > 0 && st(y, v, $, b, k, d, o, O, A, c, false), b += 200 * Math.max(v.length, 1), k = l, $++;
  }) : (F = false, st(y, _, $, b, k, d, o, O, A, c, true));
  const N = y.node().getBBox();
  S.debug("bounds", N), x && y.append("text").text(x).attr("x", N.width / 2 - p).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), u = F ? c + d + 150 : d + 100, y.append("g").attr("class", "lineWrapper").append("line").attr("x1", p).attr("y1", u).attr("x2", N.width + 3 * p).attr("y2", u).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), _t(void 0, y, ((s = o.timeline) == null ? void 0 : s.padding) ?? 50, ((h = o.timeline) == null ? void 0 : h.useMaxWidth) ?? false);
  ...
  return s = p, o;
}, "drawEvents"), jt = { setConf: r(() => {
}, "setConf"), draw: Ft }, Bt = r((n) => {
  let t = "";
...
`, "getStyles"), Vt = Wt, Gt = { db: rt, renderer: jt, parser: $t, styles: Vt };
export {
  Gt as diagram
};
