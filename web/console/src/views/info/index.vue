...
import { WEBSITE_TYPE_INDEPENDENT, WEBSITE_TYPE_ENTERPRISE, WEBSITE_TYPE_INDUSTRY, WEBSITE_TYPE_LABEL_MAP, WEBSITE_TYPE_DESC_MAP } from '@/stores/modules/enterprise'
...
const form = reactive({
	...
	template_type: (enterprise_info.value.template_type != 1 && enterprise_info.value.template_type) || 'portal',
...
const template_options = ref([
	{ label: 'AI门户', value: 'portal' },
	{ label: '豆包风格', value: 'doubao' },
	{ label: '秘塔风格', value: 'mita' },
	{ label: 'Kimi风格', value: 'kimi' },
	{ label: '独立站', value: 'independent' },
])
...
watch(() => enterprise_info.value, () => {
	...
	form.template_type = (enterprise_info.value.template_type != 1 && enterprise_info.value.template_type) || 'portal'
...
<template>
	<Layout class="px-[60px] py-8">
		<Header :title="$t('module.website_info')"></Header>
		...
		<div class="mt-5 flex-1 flex flex-col bg-white py-6 px-8 box-border" v-loading="loading">
			<ElForm ref="form_ref" class="flex-1 max-h-[calc(100vh-264px)] overflow-auto" :model="form" :rules="rules"
				label-position="top">
				<h1 class="text-[#1D1E1F] font-semibold">{{ $t('basic_info') }}</h1>
				<ElFormItem class="mt-8" :label="$t('module.website_info_logo')" prop="logo">
					<div class="mt-4 w-full flex items-center gap-4">
						<ElImage v-if="form.logo" class="h-[70px] rounded overflow-hidden" :src="form.logo"
							:preview-src-list="[form.logo]" fit="contain">
						</ElImage>
						<UploadLogo class="w-auto h-auto" v-model="form.logo" showText
							:text="$t(form.logo ? 'action_modify' : 'action_upload')" />
					</div>
					<div class="mt-2 w-full text-sm text-[#9A9A9A]">{{ $t('module.website_info_logo_tip') }}</div>
				</ElFormItem>
				<ElFormItem class="mt-8" :label="$t('module.website_info_ico')" prop="ico">
					<div class="mt-4 w-full flex items-center gap-4">
						<ElImage v-if="form.ico" class="h-[24px] rounded overflow-hidden" :src="form.ico"
							:preview-src-list="[form.ico]" fit="contain">
						</ElImage>
						<UploadIco class="w-auto h-auto" v-model="form.ico" showText cropperDisabled :allowTypeList="['ico']"
							:text="$t(form.ico ? 'action_modify' : 'action_upload')" />
					</div>
					<div class="mt-2 w-full text-sm text-[#9A9A9A]">{{ $t('module.website_info_ico_tip') }}</div>
				</ElFormItem>
				<ElFormItem :label="$t('module.website_info_name')" prop="name">
					<ElInput class="max-w-[660px]" v-model="form.name" :placeholder="$t('module.website_info_name_placeholder')"
						size="large" clearable maxlength="120" show-word-limit />
				</ElFormItem>
				<ElFormItem :label="$t('module.website_info_keyword')" prop="keywords">
					<ElInputTag class="max-w-[660px]" draggable size="large" v-model="form.keywords"
						:placeholder="$t('module.website_info_keyword_placeholder')" :max="10" :maxlength="20" />
				</ElFormItem>
				<ElFormItem :label="$t('module.website_info_desc')">
					<ElInput class="max-w-[660px]" v-model="form.desc" size="large" type="textarea" :rows="5" resize="none"
				...
				<ElFormItem :label="$t('module.website_info_copyright')" prop="copyright">
					<ElInput class="max-w-[660px]" v-model="form.copyright"
				...
				<ElFormItem :label="$t('module.website_info_language')">
					<ElSelect class="max-w-[660px]" v-model="form.language" size="large">
						<ElOption v-for="item in language_options" :key="item.value"
							:label="$t(`language_option_label.${item.value}`)" :value="item.value" />
					</ElSelect>
				</ElFormItem>
				<ElFormItem :label="$t('module.website_info_type')" prop="website_type" v-if="!isOpLocalEnv">
					<ul class="flex items-center flex-wrap gap-4">
						<li v-for="value in [WEBSITE_TYPE_INDEPENDENT, WEBSITE_TYPE_ENTERPRISE, WEBSITE_TYPE_INDUSTRY]" :key="value"
							...
							<div v-if="form.website_type === value"
							...
							<div class="text-base">{{ $t(WEBSITE_TYPE_LABEL_MAP.get(value)) }}</div>
							<div class="text-sm text-[#939499]">{{ $t(WEBSITE_TYPE_DESC_MAP.get(value)) }}</div>
						</li>
					</ul>
				</ElFormItem>
				<!-- <ElFormItem :label="$t('module.website_info_template')">
					<ul class="flex flex-wrap gap-4">
						<li v-for="item in template_options" :key="item.value"
							...
							:class="[form.template_type === item.value ? 'border-[#3664EF] text-[#3664EF]' : 'text-[#4F5052] opacity-60 pointer-events-none']"
							@click.stop="form.template_type = item.value">
							<div class="text-sm p-1.5">{{ $t(`template_option_label.${item.value}`) }}</div>
