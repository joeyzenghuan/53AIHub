...
const onSceneConfirm = () => {
	scene_form_ref.value.validate((valid: boolean) => {
		...
		if (data) {
			...
			if (index >= 0) use_scene_list.value.splice(index, 1, {
				type: 'scene',
				id,
...
<template>
	<div class="h-full overflow-y-auto relative">
		<div class="p-6 bg-white rounded">
			...
			<div class="flex flex-wrap gap-5 mt-5">
				<div v-for="(item, index) in use_case_list" :key="index"
				...
				<div class="flex-none w-[48%] h-[140px] flex-center flex-col p-5 bg-[#F7F9FC] rounded cursor-pointer"
		...
		<div class="p-6 bg-white rounded mt-6">
			...
			<div class="flex flex-wrap justify-between gap-5 mt-5">
				<div v-for="(item, index) in use_scene_list" :key="index"
					...
					<div v-else class="w-[220px] h-[205px] border flex-center flex-col mx-auto"
					...
					<template v-else>
						<el-skeleton class="mt-6" v-if="!item.scene" :rows="1" />
					</template>
				</div>
			</div>
		</div>
		...
		<el-dialog v-model="case_visible" :title="case_form.id ? $t('action_edit') : $t('action_add')" align-center
			width="600px">
			<el-form ref="case_form_ref" :model="case_form" label-width="64px" label-position="left">
				<el-form-item :label="$t('input')" prop="input_text" :rules="[...generateInputRules({ message: 'form_input_placeholder' }), { max: 200, message: $t('form_input_placeholder_max_length', { max: 200 }) }]">
					<el-input size="large" v-model="case_form.input_text" :maxlength="200" show-word-limit />
				</el-form-item>
				<el-form-item :label="$t('output')" prop="output_text" :rules="[...generateInputRules({ message: 'form_input_placeholder' }), { max: 1000, message: $t('form_input_placeholder_max_length', { max: 1000 }) }]">
					<el-input type="textarea" size="large" rows="10" resize="none" v-model="case_form.output_text" :maxlength="1000" show-word-limit />
				</el-form-item>
			</el-form>

			<template #footer>
				<div class="py-4 flex items-center justify-center">
					<ElButton class="w-[96px] h-[36px]" type="primary" @click="onCaseConfirm">{{ $t('action_confirm') }}
					</ElButton>
		...
		<el-dialog v-model="scene_visible" :title="scene_form.id ? $t('action_edit') : $t('action_add')" align-center
			width="600px">
			<el-form ref="scene_form_ref" :model="scene_form" label-width="102px" label-position="left">
				<el-form-item :label="$t('pictorial_image')" prop="image" :rules="generateInputRules({ message: 'form_upload_placeholder' })">
					<UploadImage class="!w-[120px] !h-[112px]" v-model="scene_form.image" />
				</el-form-item>
				<el-form-item :label="$t('scene')" prop="scene" :rules="[...generateInputRules({ message: 'form_input_placeholder' }), { max: 20, message: $t('form_input_placeholder_max_length', { max: 20 }) }]">
					<el-input size="large" v-model="scene_form.scene" :maxlength="20" show-word-limit />
				</el-form-item>
				<el-form-item :label="$t('description')" prop="desc" :rules="[...generateInputRules({ message: 'form_input_placeholder' }), { max: 50, message: $t('form_input_placeholder_max_length', { max: 50 }) }]">
					<el-input type="textarea" size="large" rows="5" resize="none" v-model="scene_form.desc" :maxlength="50" show-word-limit />
				</el-form-item>
			</el-form>

			<template #footer>
				<div class="py-4 flex items-center justify-center">
					<ElButton class="w-[96px] h-[36px]" type="primary" @click="onSceneConfirm">{{ $t('action_confirm') }}
					</ElButton>
