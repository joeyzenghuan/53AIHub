...
const handleEnterpriseSelect = async ({ data = {} } = {}) => {
  if (data.is_process)
    return ElMessage.warning(window.$t('apply.process'))
  if (data.is_reject)
    return ElMessage.warning(data.reject_reason || window.$t('apply.reject'))
  if (data.is_expired)
    return ElMessage.warning(window.$t('apply.expired'))
  if (data.is_loading)
    return
  ...
  if (!data.eid) {
    ...
    if (!data.eid)
      return ElMessage.warning('Invalid eid')
  }
...
<template>
  <div v-if="is_login">
    ...
    <ul v-loading="loading" class="w-[400px] max-h-[440px] pr-1 box-border overflow-auto flex flex-col gap-4">
      ...
      <li
        v-for="item in enterprise_list" :key="item.apply_id"
    ...
    <ElButton
      type="primary" text class="relative mt-4 !bg-transparent left-1/2 -translate-x-1/2"
  ...
  <ElForm v-else ref="form_ref" class="relative max-w-[440px] w-full" :model="form" label-position="top">
    ...
    <template v-if="form.type === 'mobile'">
      ...
      >
        ...
        <ElInput
          ...
          style="--el-input-bg-color: #F1F2F3; --el-input-border-color: transparent; --el-input-height: 44px;" autocomplete="new-username" name="prevent_autofill_username" size="large"
          :placeholder="$t('login.mobile_placeholder')" clearable
        />
      </ElFormItem>
      <ElFormItem
        class="relative" prop="verify_code"
      ...
      >
        ...
        <VerificationCodeInput
          ref="verify_code_input_ref" v-model="form.verify_code"
          :account="form.username"
        />
      </ElFormItem>
      <ElButton
        type="primary" round class="w-full mt-8 !h-10" :disabled="!form.username || !form.verify_code"
    ...
    <template v-else>
      ...
      >
        ...
        <ElInput
          v-model="form.username"
          style="--el-input-bg-color: #F1F2F3; --el-input-border-color: transparent; --el-input-height: 44px;" size="large" :placeholder="$t('login.mobile_placeholder')" clearable
        />
      </ElFormItem>
      <ElFormItem
        class="relative" prop="password"
      ...
      >
        ...
        <ElInput
          ...
          style="--el-input-bg-color: #F1F2F3; --el-input-border-color: transparent; --el-input-height: 44px;" size="large" type="password" show-password clearable
      ...
      <ElButton
        type="primary" round class="w-full mt-8 !h-10" :disabled="!form.username || !form.password"
        :loading="submitting" @click="onLogin"
      >
        {{ $t('action_login') }}
      </ElButton>
    ...
    <div class="flex flex-col gap-4">
      <ElButton
        v-if="form.type !== 'password'" type="default" round class="w-full !h-10 !ml-0 font-medium"
      ...
      <ElButton
        v-if="form.type !== 'mobile'" type="default" round class="w-full !h-10 !ml-0 font-medium"
        @click="onMobileLogin"
      >
      ...
      <div
        class="text-xs text-[#9A9A9A] text-center" v-html="$t('login.agree_and_policy', {
