...
const loadEnterpriseList = async () => {
	loading.value = true
	let { list = [] } = await enterprise_store.loadListData({ data: { status: -1 } }).finally(() => {
		loading.value = false
	})
...
const handleEnterpriseSelect = async ({ data = {} } = {}) => {
	...
	if (data.is_expired) return ElMessage.warning(window.$t('apply.expired'))
	...
	if (!data.eid) {
		...
		const refreshData = async () => {
			let { list = [] } = await enterprise_store.loadListData({ data: { status: -1 } })
			request_count++
...
const onLogin = async () => {
	...
	const data = await user_store.login({ type: form.type, data: form, hideError: true }).catch((err) => {
		...
		ElMessage.warning(window.$t(err.origin_message === 'unauthorized' ? 'response_message.user_not_found' : 'response_message.username_or_password_is_incorrect'))
		return Promise.reject(err)
	}).finally(() => {
		submitting.value = false
	})
	if (data.is_new_user) ElMessageBox.confirm(window.$t('login.new_user_tip', { password: `${form.verify_code}${form.verify_code}` }), window.$t('tip'), {
...
<template>
	<div v-if="is_login">
		...
		<ul class="w-[400px] max-h-[440px] pr-1 box-border overflow-auto flex flex-col gap-4" v-loading="loading">
			<!-- <ElEmpty v-if="!enterprise_list.length" class="mt-10" :description="$t('no_data')" /> -->
			<li v-for="item in enterprise_list" :key="item.apply_id"
		...
		<ElButton type="primary" text class="relative mt-4 !bg-transparent left-1/2 -translate-x-1/2"
			@click.stop="loadEnterpriseList">
			{{ $t('apply.refresh_list') }}
		</ElButton>
		<div class="flex items-center text-sm text-[#5B6A91] cursor-pointer w-max mx-auto mt-2"
	...
	<ElForm v-else ref="form_ref" class="relative max-w-[440px] w-full" :model="form" label-position="top">
		...
		<template v-if="form.type === 'mobile'">
			<ElFormItem prop="username"
				...
				<ElInput style="--el-input-bg-color: #F1F2F3; --el-input-border-color: transparent; --el-input-height: 44px;"
					v-model="form.username" autocomplete="new-username" name="prevent_autofill_username" size="large"
					:placeholder="$t('login.mobile_placeholder')" clearable />
			</ElFormItem>
			<ElFormItem class="relative" prop="verify_code"
				:rules="generateInputRules({ message: 'verification_code_placeholder' })">
				<template #label>
					<span class="text-[#1D1E1F]">{{ $t('verification_code') }}</span>
				</template>
				<VerificationCodeInput ref="verify_code_input_ref" :account="form.username"
					v-model="form.verify_code" />
			</ElFormItem>
			<ElButton type="primary" round class="w-full mt-8 !h-10" :disabled="!form.username || !form.verify_code"
				:loading="submitting" @click="onLogin">
				{{ $t('action_login') }}
			</ElButton>
		</template>
		<template v-else>
			<ElFormItem prop="username"
				...
				<ElInput style="--el-input-bg-color: #F1F2F3; --el-input-border-color: transparent; --el-input-height: 44px;"
					v-model="form.username" size="large" :placeholder="$t('login.mobile_placeholder')" clearable />
			</ElFormItem>
			<ElFormItem class="relative" prop="password"
				...
				<ElInput style="--el-input-bg-color: #F1F2F3; --el-input-border-color: transparent; --el-input-height: 44px;"
					v-model="form.password" size="large" type="password" show-password clearable
					:placeholder="$t('login.password_placeholder')" @keyup.enter="onLogin" />
			...
			<ElButton type="primary" round class="w-full mt-8 !h-10" :disabled="!form.username || !form.password"
				:loading="submitting" @click="onLogin">
				{{ $t('action_login') }}
			</ElButton>
		</template>
		...
		<div class="flex flex-col gap-4">
			<ElButton v-if="form.type !== 'password'" type="default" round class="w-full !h-10 !ml-0 font-medium"
				@click="onPasswordLogin">
				<SvgIcon class="mr-2" name="password" width="28" />
				{{ $t('login.password_login_v2') }}
			</ElButton>
			<ElButton v-if="form.type !== 'mobile'" type="default" round class="w-full !h-10 !ml-0 font-medium"
				@click="onMobileLogin">
			...
			<div class="text-xs text-[#9A9A9A] text-center" v-html="$t('login.agree_and_policy', {
	...
	<ServiceDialog v-model:visible="service_visible" />
</template>
