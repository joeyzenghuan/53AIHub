...
<template>
  <ElForm ref="form_ref" v-loading="loading" class="relative max-w-[440px] w-full" :model="form" label-position="top">
    ...
    <template v-if="active_step === 0">
      <ElFormItem
        class="is-required" prop="website_name"
      ...
      >
        ...
        <ElInput
          ...
          style="--el-input-bg-color: #F1F2F3; --el-input-border-color: transparent; --el-input-height: 44px;" size="large"
          :placeholder="$t(`login.website_name_placeholder`)" clearable
        />
      </ElFormItem>
      <ElFormItem
        class="is-required" prop="contact_name"
      ...
      <ElButton
        type="primary" round class="w-full mt-6 !h-10" :loading="submitting" :disabled="!form.website_name || !form.contact_name"
    ...
    <template v-else />
    <ElButton
      type="primary" text class="!p-0 mt-4 !mx-auto relative left-1/2 -translate-x-1/2 !bg-transparent"
      @click="onLogin"
    >
