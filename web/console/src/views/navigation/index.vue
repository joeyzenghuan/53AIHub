	...
	<Layout class="px-[60px] py-8">
		...
		<div class="flex-1 flex flex-col bg-white px-10 py-8 mt-3 box-border max-h-[calc(100vh-100px)] overflow-auto">
			...
			<div class="flex-1 overflow-y-auto bg-white rounded-lg mt-4" v-loading="tableLoading">
				<TablePlus :sortable="!filterForm.keyword" v-model:data="tableData" :total="tableTotal" style="width: 100%"
					header-row-class-name="rounded overflow-hidden" header-cell-class-name="!bg-[#F6F7F8] !h-[60px] !border-none"
					:pagination="false" @pageSizeChange="handleSizeChange" @pageCurrentChange="handleCurrentChange"
					@sortableChange="handleSortableChange">
					...
					<ElTableColumn :label="$t('navigation_is_open')" width="140" prop="status" show-overflow-tooltip>
						<template #default="{ row }">
							<ElSwitch :active-value="1" :inactive-value="0" v-model="row.status"
