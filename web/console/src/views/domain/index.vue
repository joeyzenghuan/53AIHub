...
const loadListData = async () => {
  ...
  let domain_name = (exclusive_info.value.domain || '').trim().replace(/^https?:\/\//, '').replace(/\.53ai\.com$/, '')
      ...
      <div v-loading="loading" class="flex-1 max-h-[calc(100vh-100px)] overflow-auto">
        ...
        <div class="mt-4 border rounded overflow-hidden p-6">
          ...
          <div class="w-full mt-4 flex items-center gap-3">
            <ElInput
              v-model="exclusive_domain" class="!max-w-[600px]" type="text"
              :placeholder="$t('form_input_placeholder')" disabled size="large"
            />
            <ElButton
              class="flex-none text-[#3664EF]" type="default" size="large"
              @click.stop="handleCopy({ text: exclusive_domain })"
            >
            ...
            <ElButton
              class="flex-none text-[#5A6D9E] !p-0" type="text" size="large"
              @click.stop="handleSetting({ type: 'exclusive' })"
            >
        ...
        <div class="mt-4 border rounded overflow-hidden p-6">
          <label class="text-[#1D1E1F] text-sm">
            ...
            <template v-if="independent_domain">
              ...
              <ElTag
                ...
                class="ml-3 !border-none !bg-[#E3F6E0] !text-[#09BB07] flex items-center" type="success" size="default"
              >
          ...
          <div class="w-full mt-4 flex items-center gap-3">
            <template v-if="independent_domain">
              <ElInput
                v-model="independent_domain" class="!max-w-[600px]" type="text"
                :placeholder="$t('form_input_placeholder')" disabled size="large"
              />
              <ElButton
                class="flex-none text-[#3664EF]" type="default" size="large"
                @click.stop="handleCopy({ text: independent_domain })"
              >
                <ElIcon :size="16" class="mr-2" color="#3664EF">
                  <CopyDocument />
                </ElIcon>
            ...
            <ElButton
              class="flex-none text-[#5A6D9E] !p-0" type="text" size="large"
              @click.stop="handleSetting({ type: 'independent' })"
            >
              <ElIcon :size="16" class="mr-2" color="#5A6D9E">
                <Setting />
              </ElIcon>
            ...
            <ElButton
              v-if="independent_domain" class="flex-none text-[#5A6D9E] !p-0 !ml-0" type="text" size="large"
