...
const fetchUserList = async () => {
	...
	tableData.value.forEach((item = {}) => {
		...
		if (disabled && !checkedList.value.find((row: any) => row.user_id === item.user_id)) tableRef.value.toggleRowSelection(item)
	})
	loading.value = false
}
const refresh = () => {
...
<template>
	<ElDialog v-model="visible" :title="$t('action_select')" :close-on-click-modal="false" width="800px" destroy-on-close
		...
		<div class="w-full flex items-center justify-between">
			...
			<ElInput v-model="filterForm.keyword" style="width: 220px" size="default" clearable :suffix-icon="Search"
				:placeholder="$t('module.operation_user_search_placeholder')" @change="refresh" />
		</div>
		<TablePlus ref="tableRef" class="mt-5" :data="tableData" :total="tableTotal" :page="filterForm.page"
			...
			@pageSizeChange="onPageSizeChange" @pageCurrentChange="onPageChange" @selection-change="onSelectionChange">
