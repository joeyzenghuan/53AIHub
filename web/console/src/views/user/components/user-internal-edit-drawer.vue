...
<template>
	<ElDrawer v-model="visible" :title="$t('internal_user.account.edit_member')" :close-on-click-modal="false"
		...
		<ElForm class="w-full" ref="formRef" :model="formData" label-position="top">
			<ElFormItem :label="$t('internal_user.account.name')">
				<ElInput v-model="formData.name" size="large" disabled
					:placeholder="$t('internal_user.account.name_placeholder')" />
				<div class="text-[#999] text-sm mt-2">{{ $t('internal_user.account.name_disabled_tip') }}</div>
			</ElFormItem>
			<ElFormItem class="is-required" :label="$t('internal_user.account.nickname')" prop="nickname"
				:rules="generateInputRules({ message: 'internal_user.account.nickname_placeholder' })">
				<ElInput v-model="formData.nickname" size="large"
					:placeholder="$t('internal_user.account.nickname_placeholder')" />
			</ElFormItem>

			<ElFormItem class="is-required" :label="$t('internal_user.account.mobile')" prop="mobile" :rules="[
				...generateInputRules({ message: 'internal_user.account.mobile_placeholder', validator: ['text', 'mobile'] }),
			]">
				<ElInput v-model="formData.mobile" size="large" autocomplete="new-mobile"
				...
				<div class="text-[#999] text-sm mt-2">{{ $t('internal_user.account.mobile_disabled_tip') }}</div>
			</ElFormItem>
			...
			<ElFormItem :label="$t('internal_user.account.status')" prop="status">
				<UserStatus v-model="formData.status" :userData="originalData" actionDisabled size="large" />
			</ElFormItem>
		</ElForm>
		<template #footer>
