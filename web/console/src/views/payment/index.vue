...
const handleCommand = async (command, type = '') => {
  ...
  const data = type === 'wechat' ? wechat_setting_info.value : manual_setting_info.value
  switch (command) {
    case 'setting':
      if (type === 'wechat')
        wechat_setting_ref.value.open({ data })
      else if (type === 'manual')
        manual_setting_ref.value.open({ data })
      break
    case 'enable':
    ...
    <div class="flex-1 flex flex-col bg-white py-8 px-6 mt-3 box-border">
      <div class="flex-1 h-0 max-h-[calc(100vh-240px)] overflow-auto">
        ...
        <div class="w-full mt-5 flex items-center gap-5">
          <div class="flex-1 border border-[#E6E8EB] rounded-lg box-border p-5 pb-8 group">
            <div class="relative w-full flex items-center gap-3">
              ...
              <ElTag
                v-if="wechat_setting_info.pay_status" class="!border-none !bg-[#E3F6E0] !text-[#09BB07]"
          ...
          <div class="flex-1 border border-[#E6E8EB] rounded-lg box-border p-5 pb-8 group">
            <div class="relative w-full flex items-center gap-3">
              ...
              <ElTag
                v-if="manual_setting_info.pay_status" class="!border-none !bg-[#E3F6E0] !text-[#09BB07]"
                type="success" size="default"
              >
                {{ $t('enabled') }}
              </ElTag>
            ...
            <div class="mt-3 text-sm text-[#4F5052]">
              {{ manual_setting_info.pay_setting_id ? `${$t('setting')} · ${$t('updated_at')}
							${manual_setting_info.updated_time.slice(0, 16)}` : $t('not_setting') }}
            </div>
          </div>
        </div>
        <h1 class="mt-10 font-semibold text-[#1D1E1F] opacity-60">
          USD
        </h1>
        <div class="w-full mt-5 flex items-center gap-5 opacity-60">
          ...
          <div class="flex-1 rounded-lg box-border p-5 pb-8 group" />
        </div>
      </div>
    </div>
  </Layout>
