...
<template>
  ...
  >
    <ElForm ref="form_ref" :model="form" label-position="top" @submit.prevent="() => {}">
      <ElFormItem
        :label="$t('module.platform_tool_api_key')" prop="api_key" :rules="[{ required: true, message: $t('module.platform_tool_api_key_placeholder') }, {
    ...
    <template #footer>
      <div class="py-4 flex items-center justify-center">
        <ElButton class="w-[96px] h-[36px]" type="primary" @click="handleConfirm">
          {{ $t('action_confirm') }}
        </ElButton>
