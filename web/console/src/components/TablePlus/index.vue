...
import { h, nextTick, onMounted, ref, useSlots } from 'vue'
...
<template>
  ...
  <Pagination v-if="pagination" layout="sizes, prev, pager, next, jumper" :page="page" :page-size="limit" :total="total" @size-change="onSizeChange" @current-change="onCurrentChange" />
        ...
        <div class="flex-1 overflow-y-auto">
          <div class="el-checkbox-group">
            <template v-for="field in allFieldList" :key="field.field_key">
              <el-checkbox :disabled="fixedFields.includes(field.field_key) || (showFieldList.length === 1 && field.field_key === showFieldList[0].field_key)" :checked="Boolean(showFieldList.find(item => item.field_key === field.field_key))" :value="field.field_key" @change="handleFieldChange($event, field)">
                {{ field.field_name }}
              </el-checkbox>
            </template>
          </div>
