...
import { inject, nextTick, onMounted, reactive, ref, computed } from 'vue'
...
const fetchData = async () => {
	...
	const { data: { count = 0, items = [] } } = await conversationApi[`fetch_${props.type}_conversations`]({
		...params,
	}).finally(() => {
...
<template>
	<div class="h-full overflow-y-auto bg-white rounded-lg px-8 py-7 box-border overflow-hidden" :class="className">
		<div class="flex items-center gap-2">
			...
			<ElInput class="max-w-[268px]" v-model="filterForm.keyword" size="large" clearable
				:placeholder="$t(type === 'agent' ? 'user/mobile' : 'keyword')" :suffix-icon="Search" @change="refresh" />
		</div>
		<TablePlus class="mt-5" :data="tableList" :total="tableTotal" :loading="tableLoading" :page="filterForm.page"
			:limit="filterForm.pageSize" style="width: 100%" header-row-class-name="rounded overflow-hidden"
			header-cell-class-name="!bg-[#F6F7F8] !h-[60px] !border-none" @pageSizeChange="handleSizeChange"
		...
		<div v-loading="detailLoading">
			<x-bubble-list :messages="messageList" class="px-4 relative py-4">
				...
				<template #item="{ message, index }">
					...
					<x-bubble-assistant :content="message.answer.content" :reasoning="message.answer.reasoning_content"
						:reasoning-expanded="message.answer.reasoning_expanded" :streaming="message.answer.loading">
						<template v-if="!message.answer.loading" #menu>
