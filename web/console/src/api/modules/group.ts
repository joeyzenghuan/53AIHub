...
export const GROUP_TYPE_AI_LINK = 2
export const GROUP_TYPE_AGENT = 3
export const GROUP_TYPE_INTERNAL_USER = 4
export const GROUP_TYPE_PROMPT = 5
export type GroupType =
	typeof GROUP_TYPE_USER |
	typeof GROUP_TYPE_AI_LINK |
	typeof GROUP_TYPE_AGENT |
	typeof GROUP_TYPE_INTERNAL_USER |
	typeof GROUP_TYPE_PROMPT
...
export const groupApi = {
  async list({ params = {} }: {
    ...
    const group_type = params.group_type
    delete params.group_type
    const { data = [] } = await service.get(`/api/groups/type/${group_type}`, { params }).catch(handleError)
    return data
  },
  async save({ data: { group_type, groups } }: { data: { group_type: GroupType; groups: Group[] } }) {
    const { data = {} } = await service.post(`/api/groups/type/${group_type}`, { groups }).catch(handleError)
    return data
  },
  async single_save(data: {
    ...
    const { data: resultData = {} } = await service[group_id ? 'put' : 'post'](`/api/groups${group_id ? `/${group_id}` : ''}`, data).catch(handleError)
    return resultData
  },
  ...
  async user_list(params: {
    ...
    const { data: resultData = {} } = await service.get(`/api/groups/${group_id}/users`, { params }).catch(handleError)
    return {
  ...
  async agent_list(params: {
    ...
    const group_id = +params.group_id || 0
    delete params.group_id
    const { data: resultData = {} } = await service.get(`/api/groups/${group_id}/agents`, { params }).catch(handleError)
    return {
