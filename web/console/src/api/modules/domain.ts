...
export type IndependentResolveType =
  | typeof INDEPENDENT_RESOLVE_TYPE_CNAME
  | typeof INDEPENDENT_RESOLVE_TYPE_CUSTOM
...
export type IndependentSslCertType =
  | typeof INDEPENDENT_SSL_CERT_TYPE_53AI
  | typeof INDEPENDENT_SSL_CERT_TYPE_CUSTOM
...
export const domainApi = {
  ...
  async saas_exclusive_save(domain_id: number, data: { domain: string }) {
    const result = await (domain_id
      ? service.put(`/api/saas/domains/exclusive/${domain_id}`, data).catch(handleError)
      : service.post('/api/saas/domains/exclusive', data).catch(handleError))
    const enterprise_store = useEnterpriseStore()
    enterprise_store.loadSelfInfo()
    return result
  },
  async saas_independent_save(
    domain_id: number,
    data: {
      domain: string
      config: {
    ...
    const result = await (domain_id
      ? service.put(`/api/saas/domains/independent/${domain_id}`, data).catch(handleError)
      : service.post('/api/saas/domains/independent', data).catch(handleError))
    const enterprise_store = useEnterpriseStore()
    enterprise_store.loadSelfInfo()
    return result
  },
