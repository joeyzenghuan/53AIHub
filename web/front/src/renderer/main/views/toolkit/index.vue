...
const navOffset = computed(() => {
  return props.hideHeader ? props.stickyOffset : userStore.is_login ? props.stickyOffset : 0
})
...
<template>
  <div>
    <MainHeader sticky v-if="!hideHeader">
      <template #before_suffix>
        <div class="text-base text-primary font-bold line-clamp-1 max-md:flex-1 max-md:text-center">{{
          navigationStore.toolkitNavigation.name || $t('module.toolbox') }}</div>
      </template>
    ...
    <div ref="scrollRef" class="pb-6 box-border"
      :style="{ height: hideHeader ? '100%' : '100%' }" v-loading="loading">
      <div class="w-11/12 lg:w-4/5 mx-auto">
        <div ref="headerRef"
          ...
          :class="[navOffset ? '' : 'lg:py-10']" :style="{ top: navOffset ? navOffset + 'px' : '0' }"
        ...
        >
          <el-tabs v-model="state.group_id" class="index-tabs flex-1 overflow-hidden"
            style="--el-tabs-header-height: 36px" @tab-click="handleTabChange">
          ...
          <div>
            ...
            <ElInput v-model="state.keyword" size="large" class="w-full md:hidden el-input--main"
              :placeholder="$t('toolbox.search_placeholder')" :prefix-icon="Search" />
          </div>
        </div>
        <ToolkitList class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5" :keyword="state.keyword"
          :list="links" :groupId="state.group_id" :onlyAll="onlyAll" />
      </div>
    </div>
