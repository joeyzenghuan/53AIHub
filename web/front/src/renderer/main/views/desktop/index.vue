...
<template>
  <div class="w-screen h-screen flex flex-col bg-[#F0F0F0]">
      ...
      <el-tooltip effect="light" placement="bottom-end" popper-class="!p-0 w-[300px]">
        <template #content>
          <div class="flex items-center gap-2 px-4 py-2 mt-2">
            <img class="flex-none w-10 h-10 rounded-full" :src="avatar"></img>
            <div class="flex-1 text-sm text-[#1D1E1F] truncate">{{ userStore.info.nickname ||
              $t('browser.not_logged_in') }}</div>
            <el-button v-if="!userStore.is_login" type="primary" @click="handleImmeidateLogin(true)">{{
              $t('browser.login') }}</el-button>
          </div>
          <div class="flex items-center gap-2 px-3 py-2 border-t">
            ...
            <div class="flex items-center gap-2">
              <div v-tooltip="{ content: $t('browser.leftWindow') }" class="size-6 flex-center rounded bg-[#F0EFEF] cursor-pointer" @click="handlePostion('left')">
                <svg-icon name="justify-left"></svg-icon>
              </div>
              <div v-tooltip="{ content: $t('browser.centerWindow') }" class="size-6 flex-center rounded bg-[#F0EFEF] cursor-pointer" @click="handlePostion('center')">
                <svg-icon name="justify-center"></svg-icon>
              </div>
              <div v-tooltip="{ content: $t('browser.rightWindow') }" class="size-6 flex-center rounded bg-[#F0EFEF] cursor-pointer" @click="handlePostion('right')">
                <svg-icon name="justify-right"></svg-icon>
              </div>
              <div class="h-[14px] border-r"></div>
              <div v-tooltip="{ content: $t('browser.alwaysOnTop') }" class="size-6 flex-center rounded bg-[#F0EFEF] cursor-pointer" @click="handleSetAlwaysOnTop">
                <svg-icon name="top" :color="alwaysOnTop ? '#1D1E1F' : '#C4C4C4'"></svg-icon>
              </div>
            </div>
          </div>
          <div class="flex flex-col gap-2 px-3 py-2 border-t">
            <div class="h-8 flex items-center gap-3 px-3 rounded cursor-pointer hover:bg-[#F0EFEF]"
              @click="handleTest">
            ...
            <div class="h-8 flex items-center gap-3 px-3 rounded cursor-pointer hover:bg-[#F0EFEF]" @click="handleQuit"
              v-if="userStore.is_login">
              <div class="size-4 flex-center">
    ...
    <div v-for="tab in tabs" class="flex-1 rounded-t-lg overflow-hidden " :key="tab.id"
