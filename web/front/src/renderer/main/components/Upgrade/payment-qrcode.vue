  ...
  <div>
    <ElDialog header-class="!px-5 !py-3" footer-class="!hidden" append-to-body destroy-on-close width="640px"
      ...
      <div v-else class="mt-8 mb-6 flex items-center justify-center gap-2">
        ...
        <ElButton class="min-w-[90px]" type="primary"
          size="large" @click="handlePayed">
          {{ $t('subscription.payed') }}
        </ElButton>
      </div>
    </ElDialog>
    <ElDialog center append-to-body destroy-on-close width="420px" :close-on-click-modal="false"
      v-model="success_visible" @closed="paySuccess">
      <div class="mt-8 flex flex-col items-center justify-center gap-4">
        <ElIcon size="58" color="#3ABA52">
          <SuccessFilled />
        </ElIcon>
        <span class="text-2xl text-[#000]">{{ $t(PAYMENT_TYPE.WECHAT == active_payment ? 'subscription.pay_success' : 'subscription.manual_pay_success_title') }}</span>
      </div>
