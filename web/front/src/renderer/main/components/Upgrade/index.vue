...
async function open() {
  ...
  const scrollbar_width = scrollbar_el.getBoundingClientRect().width
      ...
      <ElContainer class="h-[74vh] max-md:h-[90vh] md:!flex-row" v-loading="loading">
        ...
        <ElMain class="!flex flex-col gap-4 relative !overflow-scroll !py-8 !pl-12 max-md:!pl-5 !pr-2">
          ...
          <div class="flex-1 h-0 max-h-max relative">
            ...
            <ElScrollbar ref="version_scrollbar_ref" class="relative version-scrollbar"
              ...
              <div v-for="(item, item_index) in subscription_options" :key="item_index"
                ...
                <div :class="[subscription_options.length > 1 ? '' : 'flex flex-row']">
                  <div v-if="item.agents && item.agents.length"
                    :class="[subscription_options.length > 1 ? 'mt-6' : ' flex-1']">
                    <h2 class="text-sm font-semibold text-[#1D1E1F]">
                      {{ $t(`subscription.agent_bots_title`) }}
                    </h2>
                    <ul class="flex flex-wrap gap-3.5 mt-4">
                      <li v-for="(row, row_index) in item.agents" :key="row_index"
                        class="flex items-center gap-2 w-full" :title="row.name">
                  ...
                  <div :class="[subscription_options.length > 1 ? 'mt-5' : 'flex-1']">
                    ...
                    <div class="mt-4 flex flex-wrap items-center gap-2">
                      <ElButton class="!p-1.5" type="default" plain size="default" :disabled="!item.ai_enabled"
                        @click.stop="handleAIAssistantOpen({ type: 'windows' })">
                        <img :src="$getPublicPath(`/images/windows.png`)" class="w-4 h-4 object-cover mr-1"
                          :class="{ 'opacity-50': !item.ai_enabled }" />
                        <span class="text-xs">Windows</span>
                      </ElButton>
                      <ElButton class="!p-1.5 !ml-0" type="default" plain size="default" :disabled="!item.ai_enabled"
                        @click.stop="handleAIAssistantOpen({ type: 'ios' })">
                        <img :src="$getPublicPath(`/images/ios.png`)" class="w-4 h-4 object-cover mr-1"
                          :class="{ 'opacity-50': !item.ai_enabled }" />
                        <span class="text-xs">macOS</span>
                      </ElButton>
                      <ElButton class="!p-1.5 !ml-0" type="default" plain size="default" :disabled="!item.ai_enabled"
                        @click.stop="handleAIAssistantOpen({ type: 'chrome' })">
                        <img :src="$getPublicPath(`/images/chrome.png`)" class="w-4 h-4 object-cover mr-1"
                          :class="{ 'opacity-50': !item.ai_enabled }" />
            ...
            <ElButton v-show="scroll_left_limit > 0 && scroll_left < scroll_left_limit"
