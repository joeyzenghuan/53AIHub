  ...
  actions: {
    async loadAgentList(): Promise<Agent.State[]> {
      const fetchAgents = async () => {
        ...
        return agents.map((item: Agent.State) => {
      ...
      this.agentList = await cacheManager.getOrFetch(
    ...
    async loadCategorys() {
      const fetchCategories = async () => {
        ...
        return [{ group_id: 0, group_name: window.$t('common.all') }].concat(res.data) as Category.State[]
      }
