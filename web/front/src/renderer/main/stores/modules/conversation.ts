...
export const useConversationStore = defineStore('conversation-store', {
  ...
  actions: {
    ...
    addConversation(conversation: Conversation.Info) {
      conversation.created_at = getSimpleDateFormatString({ date: conversation.created_time, format: 'YYYY.MM.DD hh:mm' })
      conversation.updated_at = getSimpleDateFormatString({ date: conversation.updated_time, format: 'YYYY.MM.DD hh:mm' })
    ...
    async delConversation(conversation: Conversation.Info) {
      this.conversations = this.conversations.filter(item => item.conversation_id !== conversation.conversation_id)
    ...
    setCurrentState(agent_id: number, conversation_id: number) {
      ...
      if (conversation_id) {
        const conversation = this.conversations.find(item => item.conversation_id === conversation_id)
        if (!conversation) {
          conversation_id = 0
        }
      }
